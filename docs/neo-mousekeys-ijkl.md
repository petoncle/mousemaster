...
- Remove the `+` from `virtual-keys=+showrecursivehintkeys` in [neo-mousekeys-ijkl.properties](../configuration/neo-mousekeys-ijkl.properties) to replace the keys drawn in each cell with a plus over the current area and a dot per cell.
