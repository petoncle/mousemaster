...
- **`transition-animation-overshoot`**: What the indicator swells to, relative to the size it is easing to (default `1`, which is no swell). Below 1 it implodes instead, shrinking and coming back. It is a scale: the outline thicknesses, the shadow and the label follow the size, so the indicator keeps its proportions. The swell rises over the duration of the indicator being transitioned to and falls over the duration of the one it came from, so two indicators of the same size still pulse. It runs to its end even if the indicator changes again while it is rising, so a click that is over before the next frame pulses in full.
